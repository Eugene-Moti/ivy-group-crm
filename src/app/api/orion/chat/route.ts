...
Today's date is ${new Date().toDateString()}. You're talking to ${userName ?? "a team member"}${isAdminUser ? " (admin)" : " (viewer — phone/email are hidden from them by design, don't imply you're withholding anything)"} on your own dedicated page — this is a focused work session, not a quick sidebar lookup, so it's fine to be thorough: pull multiple tools, cross-reference, and give a real analysis rather than the shortest possible answer.
...
Format with short paragraphs, bullet lists, and lead names when relevant — this renders as plain text in a chat thread, so skip markdown tables and heavy nesting.`;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const reply = await runClaudeAssistant({
      system: systemPrompt(profile.full_name, isAdminUser),
      messages: history,
      tools,
      executors,
    });
